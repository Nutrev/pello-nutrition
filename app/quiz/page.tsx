...
const EVENT_TYPES: { id: EventType; label: string; desc: string; icon: string }[] = [
  { id: "road-cycling", label: "Road cycling", desc: "Sportive, gran fondo, road race", icon: "🚴" },
  { id: "gravel", label: "Gravel / MTB", desc: "Gravel race, mountain bike event", icon: "🚵" },
  { id: "triathlon", label: "Triathlon", desc: "Sprint, Olympic, 70.3, Ironman", icon: "🏊" },
  { id: "running", label: "Running", desc: "5K, 10K, half marathon, marathon", icon: "🏃" },
  { id: "trail-running", label: "Trail running", desc: "Trail race, ultra marathon", icon: "⛰️" },
  { id: "gym", label: "Gym / Strength", desc: "Lifting, CrossFit, resistance training", icon: "🏋️" },
  { id: "training-day", label: "Training day", desc: "General training session", icon: "📅" },
  { id: "recovery", label: "Recovery day", desc: "Post-race or hard session recovery", icon: "🛌" },
];
...
export default function PlannerPage() {
  ...
  const [inputs, setInputs] = useState<PlannerInputs>({
    eventType: null, durationHours: 2, intensity: "moderate",
    caffeinePreference: "moderate", budget: 50,
    dietary: [], formats: [], retailers: [], weightKg: 70,
  });
  ...
  const generatePlan = async () => {
    ...
    const prompt = `You are Pello's expert sports nutrition AI. Generate a complete, science-backed nutrition plan.

ATHLETE PROFILE:
- Event: ${inputs.eventType?.replace(/-/g, " ")}
...
What to eat/drink before starting — specific foods and products with timing

DURING EVENT (per-hour breakdown)  
Exact timing and quantities. What to take and when. Be specific about timing (e.g. "start fuelling at 30 min, then every 20-30 min")

POST-EVENT (recovery protocol)
0-30 min window, 30-120 min, and overnight recovery

PRODUCT RECOMMENDATIONS
Pick 4-6 best products from the Pello database above that match this athlete's needs, budget, formats and retailers. For each:
- Product name and brand
- How many units needed for this event
- Estimated cost
- When in the plan to use it
...
KEY NOTES
2-3 specific tips for this athlete/event combination

Be specific, practical and science-backed. Use exact quantities and timing.`;
    ...
    try {
      ...
      setRawPlan(data.plan || "Failed to generate plan. Please try again.");
    } catch {
      ...
      <div className="max-w-2xl mx-auto px-6 py-12">
        ...
        {!rawPlan && !loading && (
          <div className="mb-8">
            ...
            <p className="text-muted">Tell us about your event and we'll generate a complete pre, during and post nutrition protocol with specific product recommendations.</p>
          </div>
        )}
        ...
        {!rawPlan && !loading && (
          <>
            ...
            {/* Step 1 — Event + Duration */}
            {step === 1 && (
              <div>
                <h2 className="font-display font-semibold text-lg mb-1">What are you planning for?</h2>
                <p className="text-xs text-muted mb-5">Select your event or activity type</p>
                ...
                <button onClick={() => setStep(2)} disabled={!inputs.eventType}
            ...
            {step === 2 && (
              <div>
                ...
                <div className="card mb-4">
                  <h3 className="font-display font-semibold mb-4">Event budget</h3>
                  <div className="flex items-center gap-3 mb-2">
                ...
                <div className="bg-moss/5 border border-moss/20 rounded-xl p-4 mb-6">
                  <div className="text-xs font-mono text-moss uppercase tracking-widest mb-3">Your calculated targets</div>
          ...
          <div>
            <div className="mb-8">
              ...
              <h1 className="font-display font-bold text-3xl tracking-tight mb-3">
                {EVENT_TYPES.find(e => e.id === inputs.eventType)?.label} · {inputs.durationHours}hr
              </h1>
              <div className="flex flex-wrap gap-2">
                <span className="text-xs bg-moss/10 text-moss font-mono px-2 py-0.5 rounded-md">{carbTarget}g carbs</span>
                <span className="text-xs bg-sand font-mono px-2 py-0.5 rounded-md">{inputs.intensity} intensity</span>
            ...
            {/* AI plan */}
            <div className="card mb-6">
              <div className="text-xs font-mono text-muted uppercase tracking-widest mb-4">AI-generated plan · Pello Nutrition Intelligence</div>
              <div className="space-y-1">
                {rawPlan.split("\n").map((line, i) => {
                  if (!line.trim()) return <div key={i} className="h-2" />;
                  if (line.match(/^(PRE-EVENT|DURING EVENT|POST-EVENT|PRODUCT RECOMMENDATIONS|TOTALS|KEY NOTES|TOTAL)/i)) {
                    return <div key={i} className="text-xs font-mono text-muted uppercase tracking-widest mt-5 mb-2 pb-1 border-b border-sand">{line}</div>;
                  }
                  if (line.startsWith("- ") || line.startsWith("• ")) {
                    return (
                      <div key={i} className="flex items-start gap-2 text-sm text-muted py-0.5">
            ...
            {/* Matching products */}
            <div className="mb-6">
              <div className="text-xs font-mono text-muted uppercase tracking-widest mb-3">From the Pello database</div>
            ...
            <div className="flex gap-3">
              <button onClick={() => { setRawPlan(""); setStep(1); setInputs({ eventType: null, durationHours: 2, intensity: "moderate", caffeinePreference: "moderate", budget: 50, dietary: [], formats: [], retailers: [], weightKg: 70 }); }}
                className="btn-secondary flex-1 justify-center flex">
                Start over
              </button>
              <Link href="/products" className="btn-primary flex-1 justify-center flex text-center">
                Browse all products →
              </Link>
            </div>
          </div>
